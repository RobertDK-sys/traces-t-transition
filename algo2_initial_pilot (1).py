...
def is_prime_fast(n):
    """
    Reference primality check used only to LABEL each n for comparison
    purposes -- it plays no part in the harmonic resonance test itself.

    Instead of trial-dividing by every integer up to sqrt(n) (as a naive
    loop would), this uses the classical 6k +/- 1 structural filter: every
    prime greater than 3 has the form 6k + 1 or 6k - 1, so multiples of 2
    and 3 can be discarded immediately, and only candidates of that form
    need to be tested as divisors. This does not change the result, only
    the number of divisions performed to get there.
    """
    if n < 2:
        return False
    if n in (2, 3):
